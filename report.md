...
```
 $ cp -r /home2/amano/fortran/report ~/report
 $ cd ~/report
```
...
提出期限は2018年6月3日(日)の23:59 JST(日本標準時)とする．提出は <https://goo.gl/oNJtEx> から`tar.gz`形式のファイルをアップロードすること．
...
# 文字列処理 $^\dagger$
標準入力で与えられた入力から重複した行を取り除いた結果を表示するプログラムを作成せよ．ただし日本語等のマルチバイト文字列については考慮しなくて良い．

1行あたりの文字数や行数を任意にすると少し実装が難しくなる．自信の無い人は自分で適宜仕様(制限)を決めても良いが，制限を超えた入力があった場合にはその旨エラーを表示して終了させること．また，レポートには自分で決めた制限を明記せよ．余裕のある人は文字数や行数を任意としたプログラムに挑戦してみて欲しい．

テスト用にデータファイル `kadai5_1.txt`，`kadai5_2.txt` を用意してあるので，標準入力から与えて結果を確認せよ．例えば
...
のような結果になるハズである．(これら以外のデータであっても正しく動作するようにすること．)

## ヒント {-}
- リストを用いる方法でも良いが，ハッシュ(ハッシュテーブル)と呼ばれるアルゴリズムを用いると効率が良い．ハッシュは任意の入力値(今の場合は文字列)を整数に変換する手法である．
- 1行あたりの文字数や行数などを任意としたプログラムとするにはFortran 2003の標準モジュールである`iso_fortran_env`を用いると便利である．このモジュールの`is_iostat_end`は`read`の終了ステータス`iostat`がファイル終了かどうかを真偽値で返す関数である．同様に`is_iostat_eor`は終了ステータスから，改行かどうかを調べて真偽値を返す．同じくFortran 2003の機能である可変長文字列 `character(len=:)` を用いると便利である．
...
具体的には，サブルーチンを実装しフラグの値を変更した後に，以下のようにコンパイル・実行し，出力結果(以下の例では`kadai6.dat`)をgnuplot等を用いて図示すれば良い．ただし縦軸・横軸ともに対数スケールとすること．また，計算時間のデータサイズ$N$に対する依存性が理論的な関係性を満たすことも確認せよ．(バブルソートでは$O(N^2)$，それ以外のアルゴリズムは全て$O(N log N)$の依存性を持つ．)
...
#. $U = 0$，$D = 1$として移流拡散方程式の数値解(時間発展)を求めよ．ただし初期条件はデルタ関数$f(x, 0) = \delta (x)$とする．用いる粒子数や時間ステップに対する依存性について考察すること．
...
- 各学科は試験成績を学生のランキングとする．(同点の場合は考えなくて良い．)

とする．

このとき，学科数，学科定員，各学生の試験成績および志望学科リストのデータを読み込み，マッチングを行うプログラムを作成せよ．なおデータファイルの形式は以下のように，学科数と各学科の定員(例: 数学科，情報科学科，物理学科，天文学科，地球惑星物理学科，地球惑星環境学科，化学科，生物化学科，生物情報科学科，生物学科)，各学生について成績(実数)と志望学科が記述されている．なお，学科，学生ともに出現順にID($i = 1, 2, \ldots$)を付与し，学生の志望リストは学科IDで表す．ただし，志望学科リストに負のIDが指定されている場合は，それ以上志望学科が無いことを表す．(以下の例では学生1は第3志望までしか指定していない．)

```
10                                                      # 学科数

    4    2    7    1    3    2    4    2    1    2      # 学科定員


40                                                      # 学生数

33.92   10    4    1   -1   -1   -1   -1   -1   -1   -1 # 成績，第1志望，第2志望...
65.34    9   10    1    6    8    7    4    2    5    3 # 成績，第1志望，第2志望...
# 以下学生数分続く
```

このデータファイルを例えば以下のように読み込みマッチング結果を表示するプログラムを作成せよ．なお，全ての学生が振り分けられるわけではない(マッチングに失敗する学生がいる)ことも考慮すること．例えば`kadai8_2a.txt`に対して実行すると，以下のような結果が得られるだろう．(`kadai8_2b.txt`についても実行してみよう．)
