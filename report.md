...
提出期限は2020年6月14日(日)の23:59 JST(日本標準時)とする．提出は <https://tinyurl.com/y77b9wka> から`tar.gz`形式のファイルをアップロードすること．質問・相談はSlackで随時受け付ける．

- 提出用ファイルの作成方法

```
 $ cd ~
 $ mkdir ~/s2026xx_fortran
```

(2026xxは自分の学生証番号に合わせること)するとホームディレクトリに`s2026xx_fortran`というディレクトリが出来るので，提出したいファイルを全てこのディレクトリにコピーする．例えば

```
 $ cp fortran.pdf kadai2.f90 kadai3.f90 ~/s2026xx_fortran
```
...
```
 $ cd ~
 $ tar -zcvf s2026xx_fortran.tar.gz s2026xx_fortran
```

すればホームディレクトリに`s2026xx_fortran.tar.gz`というファイルが出来るので，これをアップロードすれば良い．
...
もし`kadai2.py`で`ImportError: No module named mpmath`などのエラーが出た場合には
```
$ sudo apt install python-mpmath
```
のように実行すればよい．
(自分でpythonをインストールした人はおそらくこれは必要ない．）
