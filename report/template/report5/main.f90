...
contains
  ...
  subroutine test_basic_units()
    ...
    q = quantity(10.0_8, "m")
    call check(q%value == 10.0_8, "m: value")
    call check(all(q%dims == [1, 0, 0]), "m: dims")

    q = quantity(5.0_8, "kg")
    call check(q%value == 5.0_8, "kg: value")
    call check(all(q%dims == [0, 1, 0]), "kg: dims")

    q = quantity(3.0_8, "s")
    call check(q%value == 3.0_8, "s: value")
  ...
  subroutine test_addition()
    ...
    b = quantity(3.0_8, "m")
    c = a + b
    call check(c%value == 5.0_8, "2 m + 3 m = 5 m")
    call check(all(c%dims == [1, 0, 0]), "addition: dims preserved")

    a = quantity(1.5_8, "kg")
    b = quantity(2.5_8, "kg")
    c = a + b
    call check(c%value == 4.0_8, "1.5 kg + 2.5 kg = 4 kg")
  end subroutine
  ...
  subroutine test_subtraction()
    ...
    b = quantity(4.0_8, "s")
    c = a - b
    call check(c%value == 6.0_8, "10 s - 4 s = 6 s")
  ...
  subroutine test_multiplication()
    ...
    b = quantity(3.0_8, "m")
    c = a * b
    call check(c%value == 6.0_8, "2 m * 3 m = 6 m^2")
    call check(all(c%dims == [2, 0, 0]), "multiplication: dims add")

    a = quantity(10.0_8, "m")
    b = quantity(5.0_8, "kg")
    c = a * b
    call check(c%value == 50.0_8, "10 m * 5 kg = 50 m*kg")
  ...
  subroutine test_division()
    ...
    b = quantity(2.0_8, "s")
    c = a / b
    call check(c%value == 5.0_8, "10 m / 2 s = 5 m/s")
    call check(all(c%dims == [1, 0, -1]), "division: dims subtract")

    a = quantity(6.0_8, "m^2")
    b = quantity(2.0_8, "m")
    c = a / b
    call check(c%value == 3.0_8, "6 m^2 / 2 m = 3 m")
  ...
  subroutine test_scalar_multiply()
    ...
    a = quantity(5.0_8, "m")
    b = 2.0_8 * a
    call check(b%value == 10.0_8, "2.0 * 5 m = 10 m")
    call check(all(b%dims == [1, 0, 0]), "scalar * quantity: dims unchanged")

    a = quantity(3.0_8, "kg")
    b = a * 4.0_8
    call check(b%value == 12.0_8, "3 kg * 4.0 = 12 kg")
  ...
  subroutine test_scalar_divide()
    ...
    a = quantity(10.0_8, "s")
    b = a / 2.0_8
    call check(b%value == 5.0_8, "10 s / 2.0 = 5 s")
  ...
  subroutine test_dimensionless()
    ...
    q = quantity(3.14_8, "")
    call check(q%value == 3.14_8, "empty string: value")
    call check(all(q%dims == [0, 0, 0]), "empty string: dimensionless")

    q = quantity(2.0_8, "1")
    call check(q%value == 2.0_8, "'1': value")
