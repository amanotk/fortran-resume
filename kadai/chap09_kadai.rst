...
サンプルプログラム :doc:`sample1.f90 </chap09_sample1_f90>` を参考にして，数学定数を定義するモジュールを作成せよ．メインプログラムも作成して，動作を確認すること．特に ``use`` によるモジュール参照， ``only`` の使い方などを理解し，定数値の書き換えが出来ない(コンパイルエラーとなる)ことを確かめよ．数学定数としては例えば :math:`\pi`, :math:`\sqrt{\pi}`, :math:`e` などを定義すれば良い．


課題3
-----

:doc:`sample5.f90 </chap09_sample5_f90>` のモジュール ``mod_vector`` で定義されている2次元のベクトル構造型 ``type(vector2)`` を拡張し，以下の演算子を定義せよ．(以下で ``a``, ``b`` は共に ``type(vector2)`` で宣言されたものとする．)
