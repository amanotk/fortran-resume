...
.. note::
   ...
   2025年現在では以下の内容はかなり古めかしい情報となってしまった．筆者は長年Emacsのヘビーユーザーであったが，現在はsshでログインしたリモートマシンのファイル編集もVS Codeで不自由なくできる時代となってしまったので，久しく ``emacs`` コマンドの実行すらしていない．ただし，キーバインディングはEmacsで使用しているし，ちょっとした(VS Codeを使うまでもない)ファイル編集にviは頻繁に利用する．そういう意味では知っておいて損になることはないだろう．
